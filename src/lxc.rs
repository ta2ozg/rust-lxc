...
pub trait LxcCommander {
    fn create(&self, name: &str, template: &str) -> Result<()>;
...
impl LxcCommander for RealLxcCommander {
    fn create(&self, name: &str, template: &str) -> Result<()> {
        let args = if template == "download" {
            vec!["-n", name, "-t", template, "--", "-d", "ubuntu", "-r", "focal", "-a", "amd64"]
        } else {
            vec!["-n", name, "-t", template]
        };
        self.execute_command("lxc-create", &args)?;
        Ok(())
    }
...
pub fn lxc_create(name: &str, template: &str) -> Result<()> {
    let commander = RealLxcCommander;
    commander.create(name, template)
}
